...
Function SLP_IXPS_UpdateZProfile(s) //HOOK FUNCTION to see spectrum at different (x,y) by moving a cursor
...
Function SLP_IXPS_DisplaySelectedData(VolName)
	...
	If (stringmatch(SLP_IXPS_ScanListW[DataRow][45],"")&&V_Value==1) //If the raw data exists and the z-profile is checked
		Cursor /I/W=SLP_ImgXPS_Panel_01#IX_IMGW/C=(65525,0,0) C $VolName round(DimSize($ImgVolW_FName,0)/2),round(DimSize($ImgVolW_FName,1)/2)
	Endif
			...
			If (ScrnMskW[i][j])
				...
				Strswitch(FittingName)	// string switch
					case "Secondary yield cut-off":
						...
						break
					case "LEEM cut-off":
						SLP_IXPS_LEEMCutOff(VolName,SVPoint)
						break
					case "1GLA":
