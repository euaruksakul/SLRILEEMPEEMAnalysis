	...
	Variable hookResult = 0
		...
		switch(s.eventCode)
			case 7:				// Activate
				...
				if (CmpStr(activeSubwindow,"SLP_DispXPS_Panel_01#DX_MergedSpectra") == 0&&!stringmatch("",CsrInfo(A,"SLP_ImgXPS_Panel_01#IX_FITTINGWIN")))
...
Function SLP_DXPS_ExpCmbnSpectrm()
	...
	If (Stringmatch(SLP_DXPS_ScanListW[SelRow][26],"0"))
	...
	ElseIf (Stringmatch(SLP_DXPS_ScanListW[SelRow][26],"1"))
		...
		//Export the spectrum wave from the data folder to the 'current' (i.e. user's) folder
		Duplicate CombinedSpectrmBEW,$Exp_CmbndSpectrmName /WAVE=Exp_CmbndSpectrm
		Note Exp_CmbndSpectrm,SLP_DXPS_CreateNotes(ScanName)
		Display Exp_CmbndSpectrm
		Modifygraph mirror=1,tick=2,minor=1
		Legend
...
Function /T SLP_Util_FindElement(PeakPos,FindRange)
