...
//	- [NEXAFS] Load scans.. button is now load only one scan at a time. It also checks and corrects the scan name automatically.
...
Function ButtonProc_DrawROI(ba) : ButtonControl
	STRUCT WMButtonAction &ba
	NVAR SLP_LL_SetScale_Sel=root:Package_SLP:Stitch:SLP_LL_SetScale_Sel
...
Function ButtonProc_CreateStitchMask(ba) : ButtonControl
	STRUCT WMButtonAction &ba
	NVAR SLP_LL_SetScale_Sel=root:Package_SLP:Stitch:SLP_LL_SetScale_Sel
...
Function SLP_Create_Stitch_Panel1()
	...
	Button button2,pos={15,195},size={80,20},title="Draw ROI",proc=ButtonProc_DrawROI
	Button button3,pos={100,195},size={100,20},title="Create mask",proc=ButtonProc_CreateStitchMask
...
//TODOs
//	- Option to load and save images as well
//	- Extraction of NEXAFS from image series
//	- Automatic ROI creation (use Particle analysis package)
//	- Fit and Global fit
...
//TODOs
//	- Manual adjust for the Disp. line
//	- BE/KE swap
//	- Peak identification
//	- Delete scans
//	- Fitting and Global Fitting
//	- Check when the raw data restore is not successful (ask user to try to locate the files)
...
Function SLP_DXPS_init()
	...
	variable /G NLines=10
	variable /G DispFactor=0.02
...
Function SLP_DXPS_CreatePanel1() : Panel
	...
	ListBox List_XPSParam,widths={45,25,20},proc=ListBoxProc_XPSParam
	...
	Button button02,pos={160,186},size={50,16},title="BE/KE"
	Button button02,fSize=10
	Button button07,pos={317,390},size={97,16},title="Fit.."
	Button button07,fSize=10
	Button button08,pos={635,573},size={110,20},title="Global fit selected.."
...
Function SLP_DXPS_DisplaySelectedData(VolName)
	...
	TraceList=TraceNameList("SLP_DispXPS_Panel_01#DX_MergedSpectra",";",1) 
	If (Exists(Indv_SpectrumW_FName))
		Wave CombinedSpectrmW=$CombinedSpectrmW_FName
	...
	Else
		Appendtograph /W=SLP_DispXPS_Panel_01#DX_MergedSpectra SLP_DXPS_DummySpectrum
	EndIf
	SLP_Util_RemoveTraces("SLP_DispXPS_Panel_01#DX_MergedSpectra",TraceList)
	ModifyGraph /W=SLP_DispXPS_Panel_01#DX_MergedSpectra lsize=2,rgb=(1,34817,52428)
	
End

Function ListBoxProc_XPSParam(lba) : ListBoxControl
	STRUCT WMListboxAction &lba
...
Function SLP_DXPS_ExpCmbnSpectrm()
	...
	String Exp_CmbndSpectrmName=ScanName+"_Cmbnd"
	
	If (Exists(Exp_CmbndSpectrmName))
		Do
			Prompt Exp_CmbndSpectrmName,"New wave name"
	...
	Duplicate CombinedSpectraW,$Exp_CmbndSpectrmName /WAVE=Exp_CmbndSpectrm
...
Function SLP_DXPS_MergeSpectra(VolName,x0,y0,DispFactor)
	...
	Make /O /N=(N_Steps) $CombinedSpectraW_FName /WAVE=CombinedSpectrmW
	...
	SLP_Util_FormatDXPS("SLP_DispXPS_Panel_01#DX_ShiftedSpectra")
	SLP_Util_FormatDXPS("SLP_DispXPS_Panel_01#DX_MergedSpectra")
...
Function SLP_Util_MakeRedGradient(SLP_GraphName)
	...
	For (i=0;i<N_Traces;i+=1)
		...
		TraceName=stringFromList(i,TraceList)
		ModifyGraph /W=SLP_DispXPS_Panel_01#DX_ShiftedSpectra rgb($TraceName)=(Grad_Offset+(65535-Grad_Offset)*(i/N_Traces),20000,20000)
	EndFor
End
		...
		case 2: // mouse up
			...
			If (V_Flag!=-1) //User select a folder
				...
				Do
					...
					DoPrompt "Enter parameters", VolName,zStart,zStep
					...
					Endif
				While (SLP_Util_WNameCheck(VolName,20)&&!V_Flag1) //Loope until user makes a good name or click Cancel
				...
				If (!V_Flag1) //User click continue
					...
					SLP_DXPS_ScanListW[N_Scans][17]=SLP_DXPS_XPSParamW[3][1]
...
Function SLP_Util_LoadMultImg2Vol(ImageFolder,VolName,zStart,zStep)
	...
	string filelist=indexedfile(SLP_ImagePath,-1,".tif")
	filelist=sortlist(filelist)
...
Function SLP_Util_FormatDXPS(SLP_GraphName)
	String SLP_GraphName
	
	ModifyGraph/Z /W=$SLP_GraphName tick=2,mirror=1,minor=1
	Label/Z/W=$SLP_GraphName left "Intensity (a.u.)"
	Label/Z/W=$SLP_GraphName bottom "Start voltage (eV)"
End
...
//TODOs
//	*Additional features	*
//	- Histogram and image brightness adjustment
//	- Th. adj. for screen mask creation
//		- Error report when scrn. msk. creation is not successful
//	- Option to choose screen mask from other scans (or anywhere else)
//	- Coef. real-time display for curve fitting
//	- Fit function selection
//	- Selectable coef. to use for mapping
//	- Est. time for extraction
//	- Quick ext. mode (no fit display)
//	- Error marking for bad fit.
//	- Size and rotation adjustments for map corrections.
//	- Change from STV to BE scale.
//	- Show ImgVolW_intg and disable setvar/slider when Raw is removed
//	- Histogram for map (determine phase populations)
...
Function SLP_IXPS_init()
	...
	SetDimLabel 1,8,FitFunction,SLP_IXPS_ScanListW
	SetDimLabel 1,9,SelMParam1,SLP_IXPS_ScanListW
	SetDimLabel 1,10,SelMParam2,SLP_IXPS_ScanListW
	...
	SetDimLabel 1,34,CorrW2,SLP_IXPS_ScanListW //Image wave FName for map2 correction
	SetDimLabel 1,35,CorrAdj_x1,SLP_IXPS_ScanListW //Map correction parameters
	SetDimLabel 1,36,CorrAdj_y1,SLP_IXPS_ScanListW
	SetDimLabel 1,37,CorrAdj_mag1,SLP_IXPS_ScanListW
	SetDimLabel 1,38,CorrAdj_Size1,SLP_IXPS_ScanListW
	SetDimLabel 1,39,CorrAdj_Rot1,SLP_IXPS_ScanListW
	SetDimLabel 1,40,CorrAdj_x2,SLP_IXPS_ScanListW
	SetDimLabel 1,41,CorrAdj_y2,SLP_IXPS_ScanListW
	SetDimLabel 1,42,CorrAdj_mag2,SLP_IXPS_ScanListW
	SetDimLabel 1,43,CorrAdj_Size2,SLP_IXPS_ScanListW
	SetDimLabel 1,44,CorrAdj_Rot2,SLP_IXPS_ScanListW
	SetDimLabel 1,45,RawDataRemoved_Flg,SLP_IXPS_ScanListW
	SetDimLabel 1,46,Notes,SLP_IXPS_ScanListW
	SetDimLabel 1,47,SelectedSubWindow,SLP_IXPS_ScanListW
	SetDimLabel 1,48,FitMode,SLP_IXPS_ScanListW
...
Function SLP_IXPS_CreatePanel1() : Panel
	...
	NewPanel /W=(200,70,1100,740) /N=SLP_ImgXPS_Panel_01
	...
	SetVariable SetVar_FrameNum,pos={Ctrl_xPos0+60,35},size={120,16},title="Frame no.",fsize=10,variable=FrameNum,proc=SetVarProc_ImgFrame
	Slider Slider_ImgVolFrame,pos={Ctrl_xPos0,55},size={216,13},limits={0,2,1},value= 0,side= 0,vert= 0,ticks= 0,proc=SliderProc_ImgFrame,variable=FrameNum
	...
	Button Button_ChooseScrnMsk,pos={Ctrl_xPos0,250},size={190,16},title="Copy screen mask from..",fSize=10

	PopupMenu Popup_FitType,pos={16,275},size={230,16},bodyWidth=180,title="Fit mode:",proc=PopMenuProc_FitSel
	PopupMenu Popup_FitType,mode=1,value= #"\"Secondary yield cut-off;2GLAs\"",fsize=12
	...
	Button Button_ChooseFitMsk,pos={282,490},size={50,16},title="Choose",fSize=10
	...
	SetVariable SetVar_ExtProgress,pos={360,490},size={100,16},title="Extraction (%)",fsize=10,variable=ExtProgress
	...
	Slider Slider_ImgVolMinInt,pos={640,630},size={145,16},variable=ColorMin,proc=SliderProc_CScale
	Slider Slider_ImgVolMinInt,limits={0,2,1},side= 2,vert= 0,ticks= 0
	Slider Slider_ImgVolMaxInt,pos={640,650},size={145,16},variable=ColorMax,proc=SliderProc_CScale
	...
	Button Button_CopyParam title="Copy values from..",pos={280,260},size={105,16},fSize=10

	
	////////////////////////////////////////////////////
	
	SetVariable SetVar_CorrAdj_x1,pos={Img_xPos1+5,Img_yPos4},size={50,16},title="x:",variable=CorrAdj_x1,limits={-inf,inf,1},proc=SetVarProc_CorrectMap1
	SetVariable SetVar_CorrAdj_y1,pos={Img_xPos1+65,Img_yPos4},size={50,16},title="y:",variable=CorrAdj_y1,limits={-inf,inf,1},proc=SetVarProc_CorrectMap1
	SetVariable SetVar_CorrAdj_mag1,pos={Img_xPos1+5,Img_yPos4+20},size={120,16},title="Int:",variable=CorrAdj_mag1,limits={0,inf,0.02},proc=SetVarProc_CorrectMap1
	SetVariable SetVar_CorrAdj_size1,pos={Img_xPos1+5,Img_yPos4+40},size={120,16},title="Size:",variable=CorrAdj_Size1,limits={0,inf,0.02},proc=SetVarProc_CorrectMap1
	SetVariable SetVar_CorrAdj_rot1,pos={Img_xPos1+5,Img_yPos4+60},size={120,16},title="Rotation:",variable=CorrAdj_Rot1,limits={-180,180,1},proc=SetVarProc_CorrectMap1
...
Function SLP_IXPS_Extract2GLAs(VolName,LLim,ULim)
	...
	If (V_Value==0)
		FuncFit /Q/W=2/NTHR=0/TBOX=768 {{line,BG_Coef},{SLP_GLA_FitFunc,GLA1_coef},{SLP_GLA_FitFunc,GLA2_coef}}  ZProfW  /D /M=ZProfMskW /C=$FitCoefConstraintsW_FName
	Elseif (V_Value==1) //Fast mode
		FuncFit /N=1/Q/W=2/NTHR=0/TBOX=768 {{line,BG_Coef},{SLP_GLA_FitFunc,GLA1_coef},{SLP_GLA_FitFunc,GLA2_coef}}  ZProfW  /D /M=ZProfMskW /C=$FitCoefConstraintsW_FName
...
Function SLP_IXPS_CreateParamMap(VolName,bin,FittingName,LLim,ULim)
	...
	strswitch(FittingName)	// string switch
		case "Secondary yield cut-off":
			N_Params=4
			ExtCmd="SLP_IXPS_ExtCutoff(VolName)"
			break
		case "2GLAs":
			N_Params=16
	...
	SLP_IXPS_SetXYScale(MapVolW_FName,FOV,xMin,xMax,yMin,yMax,Bin)
	...
	SLP_IXPS_FitConstraints(VolName)
	...
	For (i=0;i<dimsize(ImgVolW,0);i+=Bin)
		For (j=0;j<dimsize(ImgVolW,1);j+=Bin)
			If (ScrnMskW[i][j])
				...
				Strswitch(FittingName)	// string switch
					case "Secondary yield cut-off":
						SLP_IXPS_ExtCutoff(VolName)
						break
					case "2GLAs":
						SLP_IXPS_Extract2GLAs(VolName,LLim,ULim)
						break
				EndSwitch
...
Function SLP_IXPS_MakeGuessCurve(VolName,FitName)
	...
	strswitch(FitName)	// string switch
		...
		Case "2GLAs":			
			...
			variable Line_Offset=str2num(FitCoefDisplayW[0][4])
			variable Line_Slope=str2num(FitCoefDisplayW[1][4])
			Variable P1_int=str2num(FitCoefDisplayW[2][4])
			Variable P1_pos=str2num(FitCoefDisplayW[3][4])
			variable p1_fwhm=str2num(FitCoefDisplayW[4][4])
			variable p1_mix=str2num(FitCoefDisplayW[5][4])
			variable p1_as=str2num(FitCoefDisplayW[6][4])
			variable p2_int=str2num(FitCoefDisplayW[7][4])
			variable p2_pos=str2num(FitCoefDisplayW[8][4])
			variable p2_fwhm=str2num(FitCoefDisplayW[9][4])
			variable p2_mix=str2num(FitCoefDisplayW[10][4])
			variable p2_as=str2num(FitCoefDisplayW[11][4])
			
			FitZProfW=Line_Offset+x*Line_Slope+SLP_GLA_Func(p1_int,p1_pos,p1_fwhm,p1_mix,p1_as,x)+SLP_GLA_Func(p2_int,p2_pos,p2_fwhm,p2_mix,p2_as,x)
...
Function SLP_IXPS_ExtCutoff(VolName)
	...
	If (V_Value==0)
		FuncFit /W=2/Q/NTHR=0/TBOX=768 SLP_ERF FitCoefW  ZProfW /M=ZProfMskW /D /C=$FitCoefConstraintsW_FName
	ElseIf (V_Value==1) //Fast mode
		FuncFit /N=1/W=2/Q/NTHR=0/TBOX=768 SLP_ERF FitCoefW  ZProfW /M=ZProfMskW /D /C=$FitCoefConstraintsW_FName
		DoUpdate
	Endif
	...
	switch( ba.eventCode )
		case 2: // mouse up
			...
			If (Exists(ExpImgName))
				Prompt ExpImgName,"New image wave name"
				DoPrompt "Duplicate image wave name. Please enter new name:",ExpImgName
			Endif
			
			If (Stringmatch(ExpImgName,CMap1W_Name))
				duplicate /O $CMap1W_FName,$CMap1W_Name /WAVE=NewImgW
				ColorMin=str2num(SLP_IXPS_ScanListW[SelRow][20])
				ColorMax=str2num(SLP_IXPS_ScanListW[SelRow][21])
			Else
				Duplicate /O/R=[][][SelPlane] $MapVolW_FName,$ExpImgName /WAVE=NewImgW
...
Function ButtonProc_IXPS_Ext1(ba) : ButtonControl
	...
	switch( ba.eventCode )
		case 2: // mouse up
			...
			controlinfo /W=SLP_ImgXPS_Panel_01 Popup_FitType
			SLP_IXPS_CreateParamMap(VolName,bin,S_Value,LLim,ULim)
...
Function SetVarProc_CorrectMap1(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva
...
Function SetVarProc_ImgFrame(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva
	...
	switch( ba.eventCode )
		case 2: // mouse up
			...
			If (V_Flag!=-1) //User select a folder
				...
				While ((SLP_Util_WNameCheck(VolName,20)||V_Flag2)&&!V_Flag1) //Loope until user makes a good name or click Cancel
				...
				If (!V_Flag1) //User click continue
					...
					SLP_IXPS_ExtractZProfile(VolName,round(dimsize(NewImgVolW,0)/2),round(dimsize(NewImgVolW,1)/2),30)
					duplicate /O $ZProfW_FName,$ZProfMskW_FName /WAVE=ZProfMskW
					ZProfMskW=1
					
					//Make selection of the Fit Mode
					SLP_IXPS_FitInit(VolName,"Secondary yield cut-off")
					PopupMenu Popup_FitType ,win=SLP_ImgXPS_Panel_01,mode=1
					SLP_IXPS_ScanListW[N_Scans][48]="1"
					
					//Create screen mask
					SLP_IXPS_CreateScreenMask(VolName,Threshold,1000)
					
					//Display the new image volume
					SLP_IXPS_DisplaySelectedData(NewImgVolW_FName)
...
Function SliderProc_CScale(sa) : SliderControl
	STRUCT WMSliderAction &sa
...
Function SLP_Util_AddnRmTraces(SLP_GraphName,NewWFNameList,Option)
	...
	Variable Option //0=Remove all existing trace(s) and add a new one, 1=For images instead of traces
...
Function SLP_CreateLoadImg() : Panel
	...
	ListBox list0,pos={L_Margin,75},size={219,326},proc=ListBoxProc_FileList
	...
	CheckBox Preview,pos={L_Margin,425},size={54,15},title="Preview ",value=ImgLoadPreview,proc=CheckProc_LoadImg_Preview
...
Function ListBoxProc_FileList(lba) : ListBoxControl
	STRUCT WMListboxAction &lba
...
Function CheckProc_LoadImg_Preview(cba) : CheckBoxControl
	STRUCT WMCheckboxAction &cba
...
Function SLP_Util_FindElement(PeakPos,FindRange)
...
Function SLP_Util_FindAuger(PeakPos,FindRange)
	variable PeakPos,FindRange
	wave /T SLP_Auger_W
